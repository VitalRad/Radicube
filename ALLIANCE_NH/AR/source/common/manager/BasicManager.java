 ...
 **********************************************/
 ...
 //Files reserved for deletion..
 public static ArrayBlockingQueue<String> foldersToDelete = new ArrayBlockingQueue<String>(15);
 public static int MAX_WAIT_FOR_DATA_TIME = -1; //in milliseconds..

 public static AtomicInteger JOB_ARRIVAL_NO = new AtomicInteger(0);
 public static String JOBS_IN_PROG_FILE = null;
 protected static int MAX_PAUSE_JOB_READS = -1;

 protected ArrayBlockingQueue<Object[]> jobDemographicsRetriever_queue = null;
 protected static int MAX_DEMO_RETRIEVER_SIZE = -1;
 public static int DELETE_ON_EXIT = 0;
 ...
 try {
        ...
        BasicManager.MAX_WAIT_FOR_DATA_TIME = Integer.parseInt(prop.getProperty("MAX_WAIT_FOR_DATA_TIME"));

        BasicManager.JOBS_IN_PROG_FILE = prop.getProperty("JOBS_IN_PROG_FILE");
        BasicManager.MAX_PAUSE_JOB_READS = Integer.parseInt(prop.getProperty("MAX_PAUSE_JOB_READS"));
        BasicManager.MAX_DEMO_RETRIEVER_SIZE = Integer.parseInt(prop.getProperty("MAX_DEMO_RETRIEVER_SIZE"));
        BasicManager.DELETE_ON_EXIT = Integer.parseInt(prop.getProperty("DELETE_ON_EXIT"));
		...
		if( (this.logRoot == null)||
            ...
            (BasicManager.MAX_WAIT_FOR_DATA_TIME < 0)||
            (BasicManager.JOBS_IN_PROG_FILE == null)||
            (BasicManager.MAX_PAUSE_JOB_READS < 0)||
            (BasicManager.MAX_DEMO_RETRIEVER_SIZE < 0))
		    {
            ...
            /*
			String name = org.dcm4che2.tool.dcmrcv.dcm_folderCreator.generate_imageStoreName();
			if(name != null)
			  {
			   name = BasicManager.JOBS_IN_PROG_FILE+"/"+name;
			   name = utility.general.genericJobs.prepareFilePath(name, logRoot, logName);

			   boolean created = true;
			   File fname = new File(name);
			   if(!fname.exists())
				 {
				  created = org.dcm4che2.tool.dcmrcv.dcm_folderCreator.createExactlyThatLocation(name);
				  if(!created)
					{
					 System.out.println("<BasicManager.startup()> Failed to create text file location. Program cannot continue");
					 log_writer.doLogging_QRmgr(this.logRoot, this.logName, "<BasicManager.startup()> Failed to create text file location. Program cannot continue");
		             System.exit(0);
					}
				 }
			   ...
			   if(created)
				 {
                  BasicManager.JOBS_IN_PROG_FILE = name;
                  BasicManager.ARRIVING_JOBS_FILE = BasicManager.JOBS_IN_PROG_FILE+"/"+BasicManager.ARRIVING_JOBS_FILE;
                  BasicManager.ARRIVING_JOBS_FILE = utility.general.genericJobs.prepareFilePath(BasicManager.ARRIVING_JOBS_FILE, logRoot, logName);
				 }
			  }
			  */
       ...
       //=====================================================
        ...
        File srcLoc = new File(BasicManager.JOBS_IN_PROG_FILE);
        this.clearDir(srcLoc);



       //=====================================================
        this.jobDemographicsRetriever_queue = new  ArrayBlockingQueue<Object[]>(BasicManager.MAX_DEMO_RETRIEVER_SIZE);


	   //=====================================================
	     new Thread(new fileDeleteManager()).start();
	     new Thread(new jobAllocator()).start();
	     new Thread(new demoRetrieverMgr()).start();
	   //=====================================================
...
public BasicManager getThisManager()
{
 return this;
}
...
private void clearDir(File src)
{
 try
 {
  ...
  if(files != null)
    {
     for(int a = 0; a < files.length; a++)
        {
         boolean deleted = files[a].delete();
         if(!deleted)
           {
		    System.out.println("<BasicManager.clearDir()>Failed to delete File. Program cannot continue");
		    log_writer.doLogging_QRmgr(logRoot, logName,
            "<BasicManager.clearDir()>Failed to delete File. Program cannot continue");
            System.exit(0);
		   }
		}
	}
 ...
 } catch (Exception e)
 {
   ...
   log_writer.doLogging_QRmgr(logRoot, logName,
   "<clearDir> exception "+e);

   log_writer.doLogging_QRmgr(logRoot, logName,
   "<clearDir> Program cannot continue ");
 }
...
String value = null;
try {
     value = BasicManager.SITE_ID+"+"+System.currentTimeMillis()+"_"+BasicManager.jobIDcount;
			  ...
			  if(this.arrivingJobs.size() >= this.arrivingJobQueueSize)
				{
				 ...
				 log_writer.doLogging_QRmgr(this.logRoot, this.logName, "<storeInQueue_arrivingJob>   can be stored queue");
				}
			  else
				{
				 if(!this.arrivingJobs.contains(job))
...
public abstract DcmRcv get_dcmRcv();
...
//     Incharge of deleting used files.

//-------------------------------------------
class fileDeleteManager implements Runnable {
...
public void run()
{
 while(true)
 {
  try
   {
     String fileLoc = BasicManager.foldersToDelete.take();
     if(fileLoc !=  null)
       {
        if(!utility.general.genericJobs.deleteDir(new File(fileLoc)))
          {
		   log_writer.doLogging_QRmgr(logRoot, logName, "<BasicManager.fileDeleteManager()> msg: Failed to delete "+fileLoc);
		  }
	   }
   } catch (Exception e)
   {
     e.printStackTrace();
     log_writer.doLogging_QRmgr(logRoot, logName, "<BasicManager.fileDeleteManager()> exception: "+e);
   }
 }
}
...
//     Instantiates demographics retriever to do its job.

//---------------------------------------------------------
class demoRetrieverMgr implements Runnable {
...
public void run()
{
 while(true)
 {
	 ...
	 {
	  this.Data = null;
	  this.Data = (Object[]) jobDemographicsRetriever_queue.take();
	  if(this.Data == null)
	    {
	     System.out.println("<BasicManager.demoRetrieverMgr()> Null Data retrieved by demoRetrieverMgr");
	     log_writer.doLogging_QRmgr(logRoot, logName, "<BasicManager.demoRetrieverMgr()> Null Data retrieved by demoRetrieverMgr");
	    }
	  else
	    {
         new jobDemographicsRetriever((Association) this.Data[0],
								      (String) this.Data[1],
								      (String) this.Data[2],
								  	  (String) this.Data[3],
								      (String) this.Data[4],
								      ...
								      (String[]) this.Data[6],
								      (int) this.Data[7],
								      (BasicManager) this.Data[8],
								      (String) this.Data[9]).run();
		}

	 } catch (Exception e)
	 {
	   e.printStackTrace();
	   log_writer.doLogging_QRmgr(logRoot, logName, "<BasicManager.demoRetrieverMgr()> exception: "+e);
	 }
 }
}
...
//            Tasked with assigning jobs.

//---------------------------------------------------------

class jobAllocator implements Runnable {
  ...
  while(this.keepRunning)
   {
    try
    {
	 ...
	 if(repetition > 3) //can't hang about any longer..
	   {
	    fileCount++;
	    repetition = 0;
	    fileExist = false;
	   }
	 String jobTxtFileName = BasicManager.JOBS_IN_PROG_FILE+"/"+"jb_"+fileCount+".txt";
	 jobTxtFileName = utility.general.genericJobs.prepareFilePath(jobTxtFileName, logRoot, logName);
	 if(!((new File(jobTxtFileName)).exists()))
	   {
        repetition = 0;
        fileExist = false;
	    //System.out.println("FILENAME = "+jobTxtFileName);
	    BasicManager.sleepForDesiredTime(BasicManager.MAX_PAUSE_JOB_READS);
	   }
	 else
	   {
        fileExist = true;
        String[] assocData = new String[5];
	    int readCount = 0;
	    BufferedReader breader = new BufferedReader(new FileReader(jobTxtFileName));
	    String sLine = null;
	    while((sLine = breader.readLine()) != null)
	     {
		  if(readCount >= 5){break;}
		  assocData[readCount] = sLine;
		  readCount++;
	     }
		 ...
		 {
		   ecls.printStackTrace();
		   log_writer.doLogging_QRmgr(logRoot, logName, "<BasicManager.jobAllocator> elcs exception: "+ecls);
		 }

	    String[] txtSplit = assocData[1].split("<adiaha0000>");
	    if(txtSplit.length < 2)
	      {
	       System.out.println("<BasicManager.jobAllocator> alert: Retrieved data is less than length 2");
	       log_writer.doLogging_QRmgr(logRoot, logName, "<BasicManager.jobAllocator> alert: Retrieved data is less than length 2");
	      }
	    else
	      {
		   //enter, jobsDemographicsRetriever..
		   txtSplit[1] = utility.general.genericJobs.prepareFilePath(txtSplit[1], logRoot, logName);

		   Object[] argsForRetriever = new Object[10];
		   argsForRetriever[0] = null;
		   argsForRetriever[1] = txtSplit[1];
		   argsForRetriever[2] = file_extracts;
		   argsForRetriever[3] = logRoot;
		   argsForRetriever[4] = logName;
		   argsForRetriever[5] = demograhphicRetrieval_maxAttempts;
		   argsForRetriever[6] = nightHawkTags;
		   argsForRetriever[7] = demograhphicRetrieval_pauseInbetweenAttempts;
		   argsForRetriever[8] = getThisManager();
		   argsForRetriever[9] = assocData[2];

		   jobDemographicsRetriever_queue.put(argsForRetriever);
		   fileCount++;
		   repetition = 0;
		   fileExist = false;
		  }
			  ...
			  ef.printStackTrace();
			  log_writer.doLogging_QRmgr(logRoot, logName, "<BasicManager.jobAllocator()> reading exception: "+ef);
	 }
   }
}
...
public jobAllocator(String filename)
{
...
public void run()
{
  ...
  while(this.keepRunning)
   {
    try
    {
	 ...
	 if((sCurrentLine = br.readLine()) != null)
	 {
	   ...
	   String[] txtSplit = sCurrentLine.split("<adiaha0000>");
	   if(txtSplit.length < 2)
	     {
	      System.out.println("<BasicManager.jobAllocator> alert: Retrieved data is less than length 2");
	      log_writer.doLogging_QRmgr(logRoot, logName, "<BasicManager.jobAllocator> alert: Retrieved data is less than length 2");
	   ...
	   else
	     {
		  ...
		  Association thisAssoc = null;

		  int jobNum = Integer.parseInt(txtSplit[0]);
		  System.out.println("JOB NUM = "+jobNum+", file loc = "+txtSplit[1]);

		  String[] assocData = new String[4];
	      String jobTxtFileName = BasicManager.JOBS_IN_PROG_FILE+"/"+"jb_"+jobNum+".txt";
	      jobTxtFileName = utility.general.genericJobs.prepareFilePath(jobTxtFileName, logRoot, logName);
	      int readCount = 0;
          BufferedReader breader = new BufferedReader(new FileReader(jobTxtFileName));
          String sLine = null;
         ...
         argsForRetriever[7] = demograhphicRetrieval_pauseInbetweenAttempts;
         argsForRetriever[8] = getThisManager();
         argsForRetriever[9] = assocData[1];
 ...
 {
   e.printStackTrace();
   log_writer.doLogging_QRmgr(logRoot, logName, "<BasicManager.jobAllocator()> exception: "+e);
 }
