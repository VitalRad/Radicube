...
return activeJobs.toArray();
                      ...
                      if((report.equals("-100"))||(report.equals("-200")))
				      ...
				      else if(report.equals("0"))//no file
				        {
                         if(noFileCount < 2)
                           {
						    noFileCount++;
						    DcmRcv.sleepForDesiredTime(1000);
						   }
						 else
						   {
							 if(nodePos == null)
							   {
								nodePos = get_jobQueuePos(n_job.get_jobId());
							   }

							 if(nodePos != null)
							   {
								nodeDeparted = nodeIsGone(BasicManager.JOBS_IN_PROG_FILE, nodePos);
							   }


							 if(nodeDeparted)
							   {
								log_writer.doLogging_QRmgr(logRoot, logName, "conductJobExecution<"+this.n_job.get_jobId()+">.run() msg: Job completed!");
								this.checkForCompletion();

                                if(BasicManager.DELETE_ON_EXIT == 1)
                                  {
								   if(nodePos != null)
								     {
								      String jobTxtFileName = BasicManager.JOBS_IN_PROG_FILE+"/"+"jb_"+nodePos+".txt";
	                                  jobTxtFileName = utility.general.genericJobs.prepareFilePath(jobTxtFileName, null, null);
	                                  utility.general.genericJobs.deleteDir(new File(jobTxtFileName));
								     }
							      }
							   ...
							   {
								if(this.n_job.getFrom_dataWaitTime() > BasicManager.MAX_WAIT_FOR_DATA_TIME)
								  {
								   //TODO: Set to 'invisible' mode and then handle in background.
								   this.checkForCompletion();
								   log_writer.doLogging_QRmgr(logRoot, logName, "conductJobExecution<"+this.n_job.get_jobId()+">.run() msg: Job closed via timeout!");
								...
								else
								  {
								   ...
								   this.n_job.addTo_dataWaitTime(dataHandler_snoozing);
								  }
							   }
					       }
						}
				        ...
				        {
					     this.n_job.set_dataWaitTime(0); //just in case..
 ...
 public String get_jobQueuePos(String targetData)
 {
  String extractedPos = null;
  try
   {
	   String[] splitData = targetData.split("_");
	   if(splitData.length < 2)
		 {
		  System.out.println("<conductJobExecution> NODE ID SPLIT less that 2. Should never happen!");
		  log_writer.doLogging_QRmgr(logRoot, logName, "<conductJobExecution>.NODE ID SPLIT less that 2. Should never happen!");
		 }
	   else
		 {
		  int nodepos = ((Integer.parseInt(splitData[splitData.length - 1])) + 1);
		  extractedPos = Integer.toString(nodepos);
		 }

   } catch (Exception ef) {
            ef.printStackTrace();
            log_writer.doLogging_QRmgr(logRoot, logName, "<conductJobExecution>.get_jobQueuePos() error: "+ef);
   }
 return extractedPos;
 }
 ...
 public boolean nodeIsGone(String filename, String jobNum)
 {
  boolean nodeGone = false;
  try
   {
	  //============================================================================
	  //System.out.println("<nodeIsGone> FILENAME = "+filename+", JOB_NUM = "+jobNum);
	  //============================================================================

	  String[] assocData = new String[5];
	  String jobTxtFileName = filename+"/"+"jb_"+jobNum+".txt";
	  jobTxtFileName = utility.general.genericJobs.prepareFilePath(jobTxtFileName, null, null);


	  //============================================================================
	  //System.out.println("<nodeIsGone> TEXT_FILE NAME = "+jobTxtFileName);
	  //============================================================================


	  int readCount = 0;
	  java.io.BufferedReader breader = new java.io.BufferedReader(new java.io.FileReader(jobTxtFileName));
	  String sLine = null;
	  ...
	  if(assocData[3].indexOf("dead") >= 0)
		{
		 //log_writer.doLogging_QRmgr(logRoot, logName, "<conductJobExecution> THIS GUY IS GONE!>>"+jobNum);
		 nodeGone = true;
		}
    ...
    breader.close();
            ...
            e.printStackTrace();
            log_writer.doLogging_QRmgr(logRoot, logName, "<conductJobExecution>.nodeIsGone() error: "+e);
   }
 return nodeGone;
 }
...
private void doStoring(BasicJob n_job, String report)
{
 try
 {
	 ...
	 if(cuid == null)
	   {
		log_writer.doLogging_QRmgr(logRoot, logName, "<SHOULD NEVER HAPPEN> doStoring error: cuid is null for file "+report);
			  ...
			  if(n_job.get_transferMode().equals("dcmsnd")) //was transfer mode switched mid-search?
				{
				 if(this.n_job.get_status().equals("in progress"))
				   {
					log_writer.doLogging_QRmgr(logRoot, logName, "conductJobExecution<"+this.n_job.get_jobId()+">.run(failed_code:003) msg: Failed to get appropriate DcmSnd!");
...
try {
	  ...
	  if(Data == null)
	  ...
	  else
		{
		 ...
		 if(cuid == null)
		   {
		    log_writer.doLogging_QRmgr(logRoot, logName, "<SHOULD NEVER HAPPEN> check_restackQueue() error: cuid is null for file "+storedFile);
