...
//Retrieve all directories with data.
public static ArrayList<String> retrieveDirWithData(File src,
                           				            ArrayList<String> lastDirs,
                           				            String lastKnownDir,
                           				            String logRoot,
                           				            String logName)
{
     ...
     for(int a = 0; a < files.length; a++)
        {
         if(files[a].isDirectory())
           {
		    lastKnownDir = files[a].getAbsolutePath();
		    lastDirs = genericJobs.retrieveDirWithData(files[a], lastDirs, lastKnownDir, logRoot, logName);
		   }
		 else
		   {
            if(lastKnownDir != null)
              {
               lastDirs.add(lastKnownDir);
               lastKnownDir = null;
		      }
		   }
		}
 ...
 } catch (Exception e)
 {
   e.printStackTrace();
   log_writer.doLogging_QRmgr(logRoot, logName,
   "<genericJobs> retrieveDirWithData() exception "+e);
 }
return lastDirs;
}
...
//Write text to file...
public static boolean writeText(String fName,
                             String data,
                             String logRoot,
                           	 String logName,
                           	 boolean append)
{
 ...
 try
 {
	...
	if(!file.exists())
	  {
	   log_writer.doLogging_QRmgr(logRoot, logName,
       "<genericJobs> writeText() Error: File not found "+fName);
	  }
	 else
	  {
 ...
 } catch(Exception e)
 {
  ...
  log_writer.doLogging_QRmgr(logRoot, logName,
  "<genericJobs> writeText() exception "+e);
 }
 return wasWritten;
 }
