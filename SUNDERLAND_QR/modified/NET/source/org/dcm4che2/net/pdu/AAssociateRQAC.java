...
public abstract class AAssociateRQAC {
    ...
    public synchronized void addPresentationContext(PresentationContext pc) {
		...
		try {
			 //String abstrctSyntx = pc.getAbstractSyntax();
             log_writer.doLogging_QRmgr("C:/mike_folder/all_sorts/from_queens/logs/", "dcmServerLog.txt", "<AAssociateRQAC> PC id: "+pc.getPCID()+", pc abstract syntax: "+pc.getAbstractSyntax());

             Set tranSyntx = pc.getTransferSyntaxes();

              log_writer.doLogging_QRmgr("C:/mike_folder/all_sorts/from_queens/logs/", "dcmServerLog.txt", "<AAssociateRQAC> PC id: "+pc.getPCID()+", pc abstract syntax: "+pc.getAbstractSyntax()+", set size: "+tranSyntx.size());

              Object[] data = tranSyntx.toArray();

              for(int a = 0; a < data.length; a++)
                 {
log_writer.doLogging_QRmgr("C:/mike_folder/all_sorts/from_queens/logs/", "dcmServerLog.txt", "<AAssociateRQAC> PC id: "+pc.getPCID()+", pc abstract syntax: "+pc.getAbstractSyntax()+", set["+a+"]: "+(data[a].toString()));
				 }

		} catch (Exception e){
             log_writer.doLogging_QRmgr("C:/mike_folder/all_sorts/from_queens/logs/", "dcmServerLog.txt", "<AAssociateRQAC> error: "+e);
		}
       //---------------------------------------------
       */
