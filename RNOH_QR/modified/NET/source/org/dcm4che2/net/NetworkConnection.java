 ...
 * @version $Revision: 7922 $ $Date: 2008-04-01 05:03:40 -0500 (Tue, 01 Apr
 *          2008) $
 * @since Nov 24, 2005

 * On August 20, 2012, Michael Bassey & Edgar Luberenga made additions to sections of this code.
...
public class NetworkConnection {
    ...
    //=============================
    // MY ADD:
    protected boolean itsDcmRcv = false;
    protected DcmRcv dcmrcvObject = null;

    public void setDCMRCV_status(boolean value)
    {
	 this.itsDcmRcv = value;
	}


    public void set_DCMRCVobject(DcmRcv dcmrcvObject)
	{
     this.dcmrcvObject = dcmrcvObject;
	}


	public DcmRcv get_DCMRCVobject()
	{
	 return this.dcmrcvObject;
	}

    public boolean itsDCMRCV()
    {
	 return this.itsDcmRcv;
	}
                ...
                try {
                    for (;;) {
                        ...
                        if(itsDCMRCV())
                          {
		                    //============================================================
		                    log_writer.doLogging_QRmgr("C:\\dcmLogs\\", "dcmrcv.txt", "CONNECTION MADE! ");
		                    //============================================================
		                    if(dcmrcvObject != null)
		                      {
								String telHdrStr = dcmrcvObject.get_TEL_MSG();
								DataInputStream in = new DataInputStream(s.getInputStream());
								int h_counter = 0;
								while(((in.available()) < telHdrStr.length())&&
								     (h_counter < dcmrcvObject.get_maxWaitDuration()))
								 {
								  h_counter++;
								  dcmrcvObject.sleepForDesiredTime(1000);
								 }

                                if((in.available()) >= (telHdrStr.length()))
                                  {
									log_writer.doLogging_QRmgr("C:\\dcmLogs\\", "dcmrcv.txt", "GOT HERE: Point 3A ");

									byte[] dataToRead = new byte[telHdrStr.length()];
									in.read(dataToRead);
									String stringRead = new String(dataToRead);
									if(stringRead.equals(telHdrStr))
									  {
										createObject = true;
										dataToRead = new byte[4];
										int sizeRead = in.read(dataToRead);
										int hdrSize = fromByteArray(dataToRead);
										dataToRead = new byte[hdrSize];
										in.read(dataToRead);
										jobDets = (new String(dataToRead)).split("&telSprtr&");

									   //============================================================
										log_writer.doLogging_QRmgr("C:\\dcmLogs\\", "dcmrcv.txt", "GOT HERE: Point 3B ");
										if(jobDets != null)
										  {
										   for(int u = 0; u < jobDets.length; u++)
											  {
											  log_writer.doLogging_QRmgr("C:\\dcmLogs\\", "dcmrcv.txt", "jobDets["+u+"] = "+jobDets[u]);
											  }
										  }
										//============================================================
								      }
                            ...
                            if(itsDCMRCV())
                              {
                               if(dcmrcvObject == null)
                                 {
							      log_writer.doLogging_QRmgr("C:\\dcmLogs\\", "dcmrcv.txt", "DcmRcv object null <> should never happen here! = ");
							     }
							   else
							     {
							      if(createObject)
							        {
							         dcmrcvObject.checkAndCreate_NHjob(jobDets, a);
								    }
							     }

							     //============================================================
							     log_writer.doLogging_QRmgr("C:\\dcmLogs\\", "dcmrcv.txt", "GOT HERE: Point 3C ");
		                         //============================================================
						      }
